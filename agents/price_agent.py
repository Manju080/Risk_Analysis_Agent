...
def get_live_prices(tickers: list[str])->dict:
    ...
    data = yf.download(full_tickers,period="1d", auto_adjust=True, progress= False)
    ...
    for ticker,full in zip(tickers,full_tickers):
        try:
            ...
            prices[ticker]=None
    logger.info(f"Fetched live prices for {len(prices)} tickers at {datetime.now().strftime('%H:%M IST')}")
    return prices
...
def get_portfolio_value(holdings: list[dict]) -> dict:
    ...
    for e in enriched:
        e["weight_pct"] = round((e["current_value"] / total_value) * 100, 2) if total_value else 0
