...
def _fetch_returns(ticker: str, period: str = "1y") -> pd.Series:
    ...
    full_ticker = ticker if ("." in ticker or ticker.startswith("^")) else f"{
        ticker}{suffix}"

    df = yf.download(full_ticker, period=period,
                     auto_adjust=True, progress=False)

    if df.empty:
        raise ValueError(f"No price data returned for {
                         full_ticker}. Check ticker symbol.")

    prices = df["Close"].squeeze()

    # Validate — no zero prices, no >50% single-day gaps
    if (prices <= 0).any():
        raise ValueError(f"Zero or negative prices found for {
                         full_ticker} — bad data.")
